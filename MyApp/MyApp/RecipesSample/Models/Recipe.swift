...
extension Recipe {
    ...
    fileprivate static let placeholder = "placeholder"
    ...
    var thumbnailImage: Image {
        let name = imageNames.last ?? Recipe.placeholder
        return ImageStore.shared.image(name: name)
    }
    
    var smallImage: Image {
        guard imageNames.count >= 2 else {
    ...
    var fullImage: Image {
        let name = imageNames.first ?? Recipe.placeholder
        return ImageStore.shared.image(name: name)
    }
    ...
    func add(_ image: Image) -> Void {
        guard let name = imageNames.first else {
            return
        }
